...
this.canvas.id="matchcanvas";
	...
	this.resolveButton=function(event){
		...
		for( var i=0; i<Match.plate.length;i++){///while select===false?
			if((ny>Match.plate[i].y)&&(ny<Match.plate[i].y+Match.plate[i].height)&&(nx>Match.plate[i].x)&&(nx<Match.plate[i].x+Match.plate[i].width)){
				...
				//alert("selected"+i+","+nx+","+ny);
				//alert(" "+Match.plate[i].x);
				Match.offsetx=nx-Match.plate[i].x;
				Match.offsety=ny-Match.plate[i].y;
				}
			}
		}
	this.drag=function(event){event.preventDefault();///to stop the browser from scrolling or whatever, do preventDefault
		if(Match.select===true){// console.log(event.clientX);
			//this.mapReady=false;
			var nx=event.touches[0].clientX;
			var ny=event.touches[0].clientY;
			//alert("dragging"+Match.plateselected); OK
			Match.plate[Match.plateselected].setX(nx-Match.offsetx);
			Match.plate[Match.plateselected].setY(ny-Match.offsety);
	...
	this.checkTarget=function(event){
		...
		var nx=event.changedTouches[0].clientX;
	...
	this.canvas.addEventListener('touchstart', this.resolveButton,false);//function(event){alert("Xtouch at "+event.touch[0].clientX);var nx=event.touch[0].clientX;var ny=event.touch[0].clientY; Match.resolveButton(nx,ny);},false);
